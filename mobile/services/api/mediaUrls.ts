 ...
 */
export const rememberSignedUrl = (objectKey?: string | null, url?: string | null): void => {
	if (!objectKey || !url) {
		return
	}
...
export const invalidateSignedUrl = (objectKey?: string | null): void => {
	if (objectKey) {
 ...
 */
export const resolveSignedUrl = async (objectKey?: string | null): Promise<string | null> => {
	if (!objectKey) {
		return null
	}
	...
	const request = messagesApi
		.fileAccess(objectKey)
		.then(response => {
			const expiresIn = (response.expires_in ?? 900) * 1000
			leases.set(objectKey, {
				url: response.url,
				expiresAt: Date.now() + expiresIn * 0.93,
			})
			return response.url
		})
		.catch(() => {
			// 404 means the object is gone or no longer shared with this user;

...
export function MessageList({ conversationId, onScrollToTop, onRequestEdit }: MessageListProps): JSX.Element {
  ...
  const listItems = useMemo<MessageListItem[]>(() => {
    ...
    for (const message of messages) {
      ...
      if (messageDate !== lastDate) {
        items.push({
          kind: "date",
          key: `date-${messageDate}-${message.id}`,
          date: messageDate
        });
      ...
      items.push({
        kind: "message",
        key: `message-${message.id}-${message.client_message_id}`,
        message
      });
    }
  ...
  const onScroll = useCallback(() => {
    ...
    if (element.scrollTop > 140 || loadingOlderRef.current || !hasNextPage || isFetchingNextPage) {
      return;
    }
  ...
  useEffect(() => {
    ...
    for (const message of unreadMessages) {
      ...
      updateMessageDelivery(conversationId, message.id, {
        state: "read",
  ...
  return (
    <>
      <div
        ref={parentRef}
        className="relative z-[1] flex-1 overflow-y-auto px-1"
          ...
          {virtualizer.getVirtualItems().map((virtualRow) => {
            ...
            if (item.kind === "date") {
              return (
                ...
                >
                  <span className="rounded-full bg-[#f0f2f5] px-3 py-1 text-[11px] font-medium text-[#667781] shadow-sm dark:bg-[#1f2c34] dark:text-slate-300">
                    {item.date}
                  </span>
                </div>
              );
